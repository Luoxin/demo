...
export default function Info() {
    ...
    const onFinish = (values: any) => {
        console.log(values);
        axios.post("https://xihui.luoxin.live/api/admin/student/set", {
            student: {
    ...
    return (
        <div>
            ...
            >
                ...
                <Form.Item name={['student', 'name']} label="姓名" rules={[{required: true, message: '请输入姓名!'}]}
                           initialValue={state.student.name}>
                    <Input readOnly={true}/>
                </Form.Item>
                <Form.Item name={['student', 'email']} label="邮箱"
                           rules={[{required: true, type: "email", message: '请输入邮箱!'}]}
                           initialValue={state.student.email}>
                    <Input/>
                </Form.Item>
