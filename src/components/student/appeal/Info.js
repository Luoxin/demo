...
export default function Info() {
    ...
    useEffect(() => {
        if (state.type === 1) {
            console.log(1)
            axios.post("https://xihui.luoxin.live/api/examiner/appeal/get", {
                id: state.appeal.id,
                show_paper: true,
            }, {
                headers: {
        ...
        if (state.type === 2) {
            console.log(2)
            axios.post("https://xihui.luoxin.live/api/reviewer/appeal/get", {
                id: state.appeal.id,
                show_paper: true,
            }, {
                headers: {
    ...
    const onFinish = (values: any) => {
        ...
        if (state.type === 1) {
            ...
            axios.post("https://xihui.luoxin.live/api/examiner/appeal/set", {
        ...
        if (state.type === 2) {
            ...
            axios.post("https://xihui.luoxin.live/api/reviewer/appeal/set", {
                appeal_id: parseInt(state.appeal.id),
        ...
        <div>
            ...
            >
                ...
                <Form.Item name={readOne && readThree ? null : ['appeal', 'appeal_info']} label="申诉理由">
                    {/*状态不同，可编辑的不同*/}
                    <Input id="appeal_info" placeholder={info} readOnly={readOne && readThree ? true : false}
                           value={info}/>
                </Form.Item>
                <Form.Item label="申诉时间">
                    <Input value={state.appeal.create_at} readOnly={true}/>
                </Form.Item>
                <Form.Item name={readOne && readTwo ? ['appeal', 'review_info'] : null} label="批改理由"
                           initialValue={review}>
                    {/*状态不同，可编辑的不同*/}
